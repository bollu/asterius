...
data Arr a = Arr { unArr :: MutableArray# RealWorld a  }

arrReplicate :: Int -> a -> IO (Arr a)
arrReplicate (I# i) a =
  IO $ \s -> let (# s', arr #) = newArray# i a s in (# s', Arr arr #)

arrRead :: Arr a  -> Int -> IO a
arrRead (Arr arr) (I# i) = IO $ readArray# arr i

arrWrite :: Arr a -> Int -> a -> IO ()
arrWrite (Arr arr) (I# i) a =
  IO $ \s -> (# writeArray# arr i a s, () #)
...
type RandGen = IORef Int

uniform :: RandGen -> IO Double
uniform gen = return 0.5


sample2D :: RandGen -> IO Sample2D
sample2D gen = Sample2D <$> uniform gen <*> uniform gen

sample1D :: RandGen -> IO Double
sample1D = uniform
...
reflectBSDF :: RandGen -> Sphere -> Direction 'Normalized -> Position -> Int -> Direction 'Normalized -> Direction 'Normalized -> Color -> IO Color
...
radiance :: RandGen -> Ray -> Int -> Bool -> IO Color
    ...
    for_ [0..(w - 1)] $ \x -> do
       ...
       for_ [0..1] $ \sy ->
          for_ [0..1] $ \sx -> do
             ...
             old <- arrRead c i
             arrWrite c i $! (old `addColor` clampedRes)
